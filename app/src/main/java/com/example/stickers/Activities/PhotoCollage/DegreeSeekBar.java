...
public class DegreeSeekBar extends View {
  ...
  private final int mPointCount = 51;

  private int mPointColor = getResources().getColor(R.color.accentColor);
  private int mTextColor = getResources().getColor(R.color.accentColor);
  private int mCenterTextColor = getResources().getColor(R.color.accentColor);
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    for (int i = -180; i <= 180; i += 15) {
      drawDegreeText(i, canvas, i >= mMinReachableDegrees && i <= mMaxReachableDegrees);
    }
